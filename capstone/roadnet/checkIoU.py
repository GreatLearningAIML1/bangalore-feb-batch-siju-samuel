# Code for evaluating IoU - adapted from the Cityscapes scripts (https://github.com/mcordts/cityscapesScripts)
#######################
...
sys.path.append('./cityscapesScripts/cityscapesscripts/helpers')
...
args.exportFile = os.path.join(args.cityscapesPath, "evaluationResults",
                               "resultPixelLevelSemanticLabeling.json")
...
args.colorized = hasattr(
    sys.stderr,
    "isatty") and sys.stderr.isatty() and platform.system() == 'Linux'
...
def getPrediction(args, groundTruthFile):
    ...
    if not args.predictionPath:
        ...
        if not os.path.isdir(rootPath):
            printError(
                "Could not find a result root folder. Please read the instructions of this method."
            )
...
def generateMatrixTrainId(args):
    args.evalLabels = []
    for label in labels:
        if (label.trainId < 0):
            continue
...
def getIouScoreForTrainLabel(label, confMatrix, args):
    ...
    notIgnored = [
        l for l in args.evalLabels
        if not trainId2label[l].ignoreInEval and not l == label
    ]
    fp = np.longlong(confMatrix[notIgnored, label].sum())
    ...
    # return IOU
    print(trainId2label[label].name + " " + str((tp + fp)))
    print(trainId2label[label].name + " " + str((tp + fn)))
    print(trainId2label[label].name + " " + str(float(tp) / (tp + fn)))
    print(trainId2label[label].name + " " + str(float(tp) / (tp + fp)))
    return float(tp) / denom
...
def getIouScoreForCategory(category, confMatrix, args):
    ...
    labelIds = [
        label.id
        for label in labels
        if not label.ignoreInEval and label.id in args.evalLabels
    ]
    ...
    notIgnoredAndNotInCategory = [
        l for l in args.evalLabels
        if not id2label[l].ignoreInEval and id2label[l].category != category
    ]
    fp = np.longlong(confMatrix[notIgnoredAndNotInCategory, :][:,
                                                               labelIds].sum())
...
def createResultDict(confMatrix, classScores, classInstScores, categoryScores,
                     categoryInstScores, perImageStats, args):
    ...
    wholeData["averageScoreInstClasses"] = getScoreAverage(
        classInstScores, args)
    wholeData["averageScoreCategories"] = getScoreAverage(categoryScores, args)
    wholeData["averageScoreInstCategories"] = getScoreAverage(
        categoryInstScores, args)
...
def evaluateImgLists(predictionImgList, groundTruthImgList, args):
    ...
    if (not args.quiet):
        ...
        niouAvgStr = getColorEntry(getScoreAverage(
            categoryInstScoreList, args), args) + "{avg:5.3f}".format(
...
def evaluatePair(predictionImgFileName, groundTruthImgFileName, confMatrix,
    ...
    if (CSUPPORT):
        # using cython
        confMatrix = addToConfusionMatrix.cEvaluatePair(predictionNp,
                                                        groundTruthNp,
                                                        confMatrix,
                                                        args.evalLabels)
    else:
    ...
    if args.evalPixelAccuracy:
        ...
        notIgnoredPixels = np.in1d(groundTruthNp, notIgnoredLabels,
